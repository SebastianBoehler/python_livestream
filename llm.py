...
def generate_and_save_tts_content(
    topic: str = "latest finance and crypto news",
    output_dir: str = None,
    filename: str = None
) -> str:
    """
    Generate news content and save it to a file for TTS processing.
    
    Args:
        topic (str): The news topic to generate content about
        output_dir (str, optional): Directory to save the content file
        filename (str, optional): Filename for the content file
    ...
    try:
        # Generate content
        content = generate_news_content(topic=topic)
        
        # Determine output directory
        if not output_dir:
            output_dir = os.path.join(os.getcwd(), 'content')
        ...
        if not filename:
            ...
            filename = f"news_content_{timestamp}.txt"
        ...
        logger.info(f"News content saved to: {output_path}")
        return output_path
        
    except Exception as e:
        logger.error(f"Error generating and saving content: {str(e)}")
        raise
